...
description: 本文将告诉大家如何使用 Avalonia 官方提供的 Headless 库，在 Docker 容器中无需配置 xvfb 运行环境、无需手动实现复杂的离屏渲染适配逻辑，即可快速实现 Avalonia 应用的渲染输出能力。和我之前写的 Avalonia 在 Docker 容器中通过 xvfb 运行的方法提到的实现方式相比，这种方案依赖更少、部署更简单，非常适合服务端纯渲染的业务场景。
tags: Avalonia
category: 
---
