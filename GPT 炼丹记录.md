...
```
请帮我将以下内容转述为地道的英文：如果某次的 Action 构建失败了，等我下次 commit 将其修复了，现在是没有发送邮件通知我原本失败的 Action 已经被 Fixed 了。我期望要这样的功能，即当 Action 的执行结果状态变更的时间，无论是从成功变成失败、还是从失败变成成功，都能通过邮件通知到我。如果我某次 Action 是失败的，那么等我推送下次的 commit 的时候，我不知道能否修复问题，如果没有邮件通知，那我就需要盯着 Action 运行，我感觉这是需要优化的
```

```
请帮我将以下内容转述为地道的英文：

我的某一个属性是可能被绑定，但更多时候用户都不关注这个属性，意味着这个属性的变更不是所有用户都关注的。但是我对这个属性的赋值是耗时的。我期望只是通知这个属性的变更，等待这个属性的 getter 求值时，再进行 Lazy 的求值。我发现在 Avalonia 里面只有 SetAndRaise 和 RaisePropertyChanged 方法，这两个方法都需要立刻传入新的值。是否在 Avalonia 里面有一个只通知属性变更，而不需要立刻给属性赋值的方法？
```

```
请帮我将以下内容转述为地道的计算机英文：

我期望的是只是通知属性变更，而不立刻给出变更之后的值。如果有业务关心这个属性的变更，再交给业务 Lazy 求值
```
