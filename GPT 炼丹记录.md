...
```
请帮我将以下内容转述为地道的英文：经过了我的调查，我认为当前的问题是系统渲染的问题。当窗口样式如当前描述的状态时，切换颜色时，将会从渲染层移动窗口的内容。那为什么设置了 ThemeMode 之后就只会看到 shaking 效果？那是因为当设置 ThemeMode 之后，将会在当前窗口的资源里面塞入主题资源，变更系统颜色时将会触发主题资源变更，进而导致全窗口刷新。窗口刷新时将会带来窗口重新渲染，窗口重新渲染将会更新渲染界面，从而修复界面渲染内容。人类可以看到的效果就是，先在变更系统颜色时，由系统底层渲染窗口内容向左偏移，随后 WPF 框架将变更主题资源，重新渲染界面，从而让界面再次正确地渲染出来，这就是 shaking 效果的原因
```

```
请帮我将以下内容转述为地道的计算机英文：由于我只有一台机器，刚好这台机器我还在开发过程中，我可以观察到开启 4 个窗口时，我的机器性能已经不足了。此时的性能差异我很难评估，因为这取决于我的后台还有什么应用在跑。无论是 x86 还是 x64 平台，我的帧率都不足 30 帧，一旦我的后台任务稍微做点事情，我的帧率能降到更低，甚至不足 10 帧。似乎一旦超过机器所能承受的上限，渲染帧率就会快速下降
```

```
请帮我将以下内容转述为地道的计算机英文：经过了本次实验，我也许知道了你的问题所在。正常来说，使用 x64 版本时，会占用更多的资源。而此刻刚好超过了计算机所能承受的上限，这也就是为什么使用x64时，其表现会不如x86的原因
```
...
现在世界上有很多人都在关注着 WPF 开源项目的发展。然而我发现了现在 GitHub 上的开源仓库最近很长一段时间都没有活跃，我期望能了解到一些信息。或者是能够直接在 https://github.com/dotnet/wpf/discussions/11188 上回复
```
