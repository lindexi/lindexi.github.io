...
很抱歉，我尝试了很长时间，但是我依然写不出来任何的测试代码。我需要你的帮助。我遇到的问题是调用 SetBackground 时，将作用于最终渲染到窗口上的位图，这将导致我无法使用 RenderTargetBitmap 的方式。我在 Uno.UI.RuntimeTests.Skia 单元测试项目尝试使用 GtkHost 类型失败，因此我也无法使用 GtkHost.TakeScreenshot 方法进行截图。我现在不知道我应该将测试代码写到哪个文件里面，也不知道可以如何对此进行测试。我十分期望能得到你的指导
```
