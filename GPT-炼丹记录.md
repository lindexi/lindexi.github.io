...
1. 在 XInput 里面，触摸的尺寸不再使用宽度和高度表示，而是采用椭圆的长轴和短轴表示。
2. 是否对宽度和高度需要进行 DPI 缩放计算？
3. 在 XInput 里面，如果触摸的尺寸和压感没有变化，那将收不到值，可能需要在底层采用上一次触摸 event 的数据。现在我的做法是没有收到值时，返回默认 0 值
```
