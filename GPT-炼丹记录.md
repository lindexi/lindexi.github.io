...
由于内存测量是一个复杂的话题，为了避免咱的认知不相同导致的误解，我十分推荐你尝试一下一个足够简单的应用程序。比如我下面为你提供的代码是一个放在 Program.cs 的通过控制台启动的 WPF 窗口的程序。这是一个十分简单的应用程序，请你确保切换到 x86 平台下，使用 Release 发布后运行。运行之后你可以尝试看一下这个进程占用的内存的大小。为了防止对你造成困扰，我还将我的项目的所有代码上传到 GitHub 上，你可以下载我的项目的所有文件，在 VisualStudio 里面打开 HayceewiballfergeBawbalhega.sln 文件，接着切换到 Release 模式构建且脱离 VisualStudio 在 Explorer 里双击 exe 文件运行应用
```
