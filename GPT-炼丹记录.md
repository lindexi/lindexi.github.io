...
我正在回到这个工作上面，在最近的工作中，我成功为 Avalonia 添加了触摸压感的支持。触摸压感的支持是做书写应用的基础前提。我依然认为 Ink 模块是需要有 UI 框架的内置支持的，因为在实现 Ink 过程中，整个 UI 框架都必须围绕高性能和高响应的目标而进行调整，这将有利于 Avalonia 框架的能力提升，让 Avalonia 更加强大。我最近十年都在工作于书写应用，特别是大尺寸的触摸屏设备上的书写应用，书写应用包含了许许多多专业的知识。我深刻的意识到想要制作一个合格的书写应用，那么 UI 框架的支持是不可或缺的
```
