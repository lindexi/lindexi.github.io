...
```
请帮我将以下内容转述为地道的英文：我将 demo 的窗口设置为背景透明且输入穿透，再设置 demo 的窗口覆盖在 Avalonia 的窗口上方。在 Avalonia 的窗口接收输入事件，随后将输入事件一边设置 Avalonia 的控件移动，另一边给到 demo 窗口，让 demo 窗口移动黄色的矩形。我尝试用鼠标拖动 Avalonia 窗口的蓝色矩形，此时 demo 的黄色矩形也会跟随着移动，我观察到当前的 demo 的黄色矩形能够更加低延迟地跟随鼠标。如此可以证明当前 Avalonia 的 LowLatencyDxgiSwapChain 模式没有按照你的想法实现，当前的延迟依然是很高的
```
