...
我认为这里有质的不同。在 WinUI 里面，我猜它不会渲染空白的帧。而在 UNO 里面是有渲染空白帧的情况。我认为在界面还没准备好之前，不应该进行渲染。执行渲染也会耗费计算资源，与其浪费计算资源，不如将这些计算资源给到界面布局等逻辑使用。额外的，这里面有一个隐藏的问题，那就是 UNO 的 UI 线程不是 X11 的 UI 线程，因此这两个线程之间存在同步的问题
```
...
如果我期望在布局完成之后再显示窗口，我是否有方法可以实现？我不在意窗口显示时间的快慢，但是我很关注应用软件的视觉效果。额外补充，splash screen 是难以满足我的需求的，因为我的应用从设计上就不要 splash screen 且不允许有 splash screen
```
