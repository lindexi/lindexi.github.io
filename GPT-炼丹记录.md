...
更进一步的问题，我认为在 UNO 框架里面，将线程分为 X11 线程和 UNO 主线程两个线程是一个不合理的设计。每个窗口都会有一个 X11 线程这样的设计也是不合理的。当 UI 线程无法及时处理完输入消息时，那将会导致消息堆积。即使正确实现了 GetIntermediatePoints 功能，也会由于输入消息使用更高优先级调度到 UI 线程，导致渲染消息迟迟无法推送，从而导致界面停止渲染
```
