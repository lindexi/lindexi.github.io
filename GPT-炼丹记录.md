...
```
请帮我将以下内容转述为地道的英文：你对渲染延迟的建议是什么呢？或者说你对如何在 Avalonia 上创建一个笔迹应用的建议是什么呢？我尝试探索了很多年了，但直到现在我都还没有一个完全在 Avalonia 上实现的方案。如果只是简单收集触摸点绘制一段路径，那无疑这是一个很粗糙的应用，我所需要的是一个有竞争力的、能够在实际产品上使用的技术方案
```

```
请帮我将以下内容转述为地道的计算机英文：经过我的调查，这个问题的核心是 WPF 在选择文本时，没有给 UIA 做通知。这更像是一个功能点没有实现，而不是 Bug
```
