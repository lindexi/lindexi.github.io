...
```
请帮我将以下内容转述为地道的英文：根据我阅读了此问题的描述内容，我不认为你将这个问题放在本仓库是正确的。根据此问题的描述内容，我认为这更像是一个系统层的问题，而不是某个 UI 框架能够导致的问题。但很抱歉，我不知道这个问题应该在哪里报告才是正确的
```
...
```
请帮我将以下内容转述为地道的计算机英文：在 GetOriginOffsetsLogical 方法里，期望返回的是相对于当前屏幕的坐标系，然而由于 Windows 的机制，在非主屏下，将返回相对于主屏的坐标。这将导致出现 #8517 问题。本修复是在计算过程中，通过减去 displayRect 的 Left 和 Top 使得计算出来的坐标是相对于当前屏幕的，而非相对于主屏，从而修复触摸偏移问题
```

```
请帮我将以下内容转述为地道的计算机英文：对于绝大部分用户来说，这是没有任何影响的。因为绝大部分用户只有单个触摸屏，此时的 displayRect 的 Left 和 Top 将都是 0 的值，这就意味着 `point.x - displayRect.left, point.y - displayRect.top` 不会对 point 的 x 和 y 产生任何变更
```
