...
我发现在 PointerTouchDevice 的实现里面，将使用一个特别奇怪且不正确的方式获取触摸点的尺寸。在 PointerTouchDevice 的 GetStylusPointWidthOrHeight 方法里面，将会尝试取出触摸点的宽度和高度信息，然而它使用了 Resolution 进行换算，如此将得到触摸点的物理尺寸。请特别关注这一点，这里拿到的是现实触摸的物理尺寸。紧接着使用拿到的现实触摸的物理尺寸与 `pixelsPerInch` 进行计算，然而此 `pixelsPerInch` 表示的是显示屏幕的屏幕 DPI 值，这无法与现实物理尺寸关联。进而导致在 PointerTouchDevice 里面返回一个十分奇怪的且不正确的触摸尺寸。核心原因是混淆了屏幕 DPI 和触摸的现实物理尺寸的含义
...
我请来了我的伙伴，借到了另一个触摸屏，组装了一个带两个触摸屏的设备。我在 Kylin (debian bullseye/sid) 系统上进行测试。经过我的测试，X11 将会自动在多个屏幕里，根据屏幕的分辨率，对输入的触摸 TouchMajor 进行缩放。因此获取的 Monitor 为触摸所在的 Monitor ，即可使用Monitor的分辨率计算触摸像素的值。但是 X11 里面的行为很类似 Windows 上没有开启 PM 的 DPI 感知的情况，受限于 X11 的能力，我的代码不能如实反映在多个触摸屏幕上的物理面积，但这是 X11 的限制。我建议等后续有其他开发者有多屏触摸需求时，再对此行为进行详细的讨论。至少我现在的代码状态可以在一个触摸屏上正常工作
```
...
根据我的分析，这是因为当使用 D3D9On12 时，将原本应该统计或存放在显卡内存里的内存统计到 WPF 应用程序里面。如以下几个堆栈所示，当 `wpfgfx_cor3.dll!CD3DDeviceManager::CreateNewDevice` 创建设备时，所创建的资源在 `igdgmm64.dll` 将申请在当前 WPF 应用程序里面。由于我无法从 https://software.intel.com/sites/downloads/symbols 拉取到符号，我不知道 igdgmm64.dll 调用了哪些函数
```

```
请帮我将以下内容转述为地道的计算机英文：

这是一个令人激动的功能，特别感觉开发团队的辛勤付出。我看到了 UWP 已经支持 .NET 9 的 AOT 了，那我想问问什么时候可以让 WPF 也支持 .NET 的 AOT ? 特别期待 WPF 也能使用到 AOT 技术。我看到在 GitHub 上有社区的伙伴提交了很多对 C++/CLI 到 C# 迁移的代码，然而 WPF 团队已经很久无法投入精力和资源到这些代码的审查上了，期望微软能够给 WPF 团队更多的资源，让 WPF 能够更好的发展
```
