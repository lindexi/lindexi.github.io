...
在类的开始先引用`using System.Management;` 然后可以使用下面代码找到硬件序列号
...
这里的 key 可以使用多个不同的字符串，如需要找到 CPU 的序列号，就可以使用 `Win32_Processor` 在另一个博客 [C# 获取 PC 序列号](https://lindexi.oschina.io/lindexi/post/C-%E8%8E%B7%E5%8F%96-PC-%E5%BA%8F%E5%88%97%E5%8F%B7.html ) 可以看到如何拿到序列号

可以使用的字符串请看本文最后

在创建 ManagementObjectSearcher 之后就可以使用`Get`方法拿到 `ManagementObject` ，这个对象里面就有很多属性。例如获得序列号就可以通过如下面代码拿到
...
如果需要遍历所有的属性就可以使用上面的 `temp` 遍历拿到，建议在 VisualStudio 遍历，这样变量的值就可以看到
