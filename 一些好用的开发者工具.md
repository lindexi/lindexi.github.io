...
![](<.gitbook/assets/一些好用的开发者工具3 (1).png>)
...
下载地址： [http://www.nirsoft.net/utils/gdi\_handles.html](http://www.nirsoft.net/utils/gdi_handles.html)
...
推荐使用方法，将下载的 Err\_x.x.x.exe 文件重命名为 error.exe 接着将此文件所在的文件夹，加入到环境变量的 Path 中。如此即可后续在命令行里面输入 `error 错误码` 比如说错误码为 `c000021a` 输入完成之后的命令行输出大概如下
